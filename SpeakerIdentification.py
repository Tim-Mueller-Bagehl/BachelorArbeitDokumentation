...
class SpeakerIdentificationSystem:
    ...
    def addFile(self,audioFile:str,ID:str):#Testfunction
        #audioFileLocation = TempDirectory / audioFile
        embedding = self.convertAudioFileToEmbedding(audioFile)
        self.addEmbedding(embedding,ID)

    def checkFile(self,audioFile : str,add):#Testfunction
        embedding = self.convertAudioFileToEmbedding(audioFile)
        return self.searchIndex(embedding,add=add)


    def manageVoicerecognition(self,audioFile : str):
        """Manages the whole Voicerecognition Pipeline. A new ID is generated and a new MemoryIndex is created if the Person can't be Identified.
        Returns the extracted text and the ID of the person.
